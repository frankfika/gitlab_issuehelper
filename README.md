...
```
输入描述 → 添加截图（可选）→ 点击生成 → 复制或提交到 GitLab
```

1. **输入描述**：在左侧文本框描述你遇到的 Bug 或需要的功能
2. **添加截图**（可选）：使用 Ctrl+V 粘贴截图，或拖拽图片到上传区域
3. **生成 Issue**：点击「Generate Issue」按钮，AI 会自动生成规范的 Issue
4. **复制或提交**：复制内容到剪贴板，或配置 GitLab 后直接提交

### 配置 GitLab 项目

支持配置多个 GitLab 项目（包括自建 GitLab 实例）：

1. 点击页面右上角 **「Configure」** 按钮
...
2. 从下拉框选择目标项目
3. 点击 **「Submit to GitLab」** 按钮
4. 提交成功后会显示 Issue 链接，点击可直接跳转查看
